...
describe('DragonchainRequestObject', () => {
  describe('#constructor', () => {
    it('assigns default instance vars', () => {
      const dro = new DragonchainRequestObject('PUT', '/chains/smartcontract', 'myDcid', 'hi!')
      expect(dro.version).to.equal('1')
      expect(dro.url).to.equal('https://myDcid.api.dragonchain.com/chains/smartcontract')
      ...
      expect(dro.message).to.equal('hi!')
